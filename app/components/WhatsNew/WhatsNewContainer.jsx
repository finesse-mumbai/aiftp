...
import { faUsers, faArrowRight, faFileAlt, faUserFriends } from '@fortawesome/free-solid-svg-icons';
...
function WhatsNewContainer() {
  return (
    <div className="min-h-screen bg-white relative flex items-center justify-center border">
        ...
        <div className='mx-auto w-full max-w-screen-2xl grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-10 px-2 md:px-24 mt-8 md:mt-16'>
        {whatsNewItems.map((item, index) => (
                <WhatsNewCard key={index} title={item.title} newtext={item.label} icon={item.icon} />
            ))}
        </div>
    </div>
  )
